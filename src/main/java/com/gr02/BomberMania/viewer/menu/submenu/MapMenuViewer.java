...
public class MapMenuViewer extends Viewer<MapMenu> {
    ...
    protected void drawElements(GUI gui) {
        gui.drawText(new Position(1, 1), "Please select the chosen map:", "#FFFFFF");


        for (int i = 0; i < getModel().getNumberEntries(); i++) {
            gui.drawText(new Position(4, 5+i), getModel().getEntry(i), getModel().isSelected(i) ? "#FFD700" : "#FFFFFF");
        }
    }
}
